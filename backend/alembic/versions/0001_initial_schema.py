...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("parent_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey(f"{SCHEMA}.tags.id"), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column("user_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey(f"{SCHEMA}.users.id"), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column("blog_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey(f"{SCHEMA}.blogs.id", ondelete="CASCADE"), primary_key=True),
        sa.Column("tag_id", postgresql.UUID(as_uuid=True),
                  sa.ForeignKey(f"{SCHEMA}.tags.id"), primary_key=True),
        schema=SCHEMA,
    )


def downgrade() -> None:
    op.drop_table("blog_tags", schema=SCHEMA)
    op.drop_table("drafts", schema=SCHEMA)
    op.drop_table("blogs", schema=SCHEMA)
